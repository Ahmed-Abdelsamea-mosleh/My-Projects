...
/*filter 3: invert images
 looping over the main image to determine each pixel's R , G , B and put it in a variable then inverting
 pixels by subtracting the values of R G B from 255 then print the modified image */


/* filter 5 : flip image
 let the user choose between fliping horizontsly or vertically
 horizontally : we will loop over the image and declare a variable and make it equal to the main image
 then equating the main image pixels from left to image pixel from right , the equatimg image pixel from right to the
 variable we declared in the begining
 vertically : same as horrizontally but we change left and right with top and bottom respectivily
 */



/*filter 8 : crop image
let the user  enter from where to begin x, y and the dimensions (w, h) of the area we want to crop .
Create a new Image object named newImage with dimensions w x h.
Iterate over each pixel:
    Loop in the xaxis from 0 to w - 1.
    Loop in the yaxis from 0 to h - 1.
    For each pixel, iterate over the color channels (R, G, B):
    Copy the pixel values from the original image to the corresponding position in newImage.
           - The position in the original image is (x + i, y + j). where i and j are 0 and 0 in the begining of the loop
           - The position in newImage is (i, j).
 */
...
computeGaussianKernel : This function calculates the weights for a Gaussian kernel,
which is a key component of blurring algorithms. The Gaussian kernel is a matrix used to
apply the blur effect. It calculates the weights based on the provided radius and sigma
values, which determine the size and strength of the blur effect.

applyGaussianBlur: This function applies the Gaussian blur to the image. It first computes
the Gaussian kernel using the computeGaussianKernel function. Then, it creates a
temporary image to store the intermediate results of the horizontal blur pass. It iterates
over each pixel in the image, applying the horizontal blur by convolving the image with the
Gaussian kernel. After completing the horizontal blur pass, it applies the vertical blur pass
using the same kernel and the temporary image. This two-pass approach is used to
separate the horizontal and vertical blurs, which improves performance and accuracy.

main: In the main function, an image is loaded from the file "building.jpg". This image is
then passed to the applyGaussianBlur function to apply the blur effect. Finally, the blurred
image is saved to a file named "blurredImage.jpg".

Additionally, the constant blurRadius is set to 50, which determines the extent of the blur
effect. Higher values of blurRadius result in a stronger blur effect but also increase the
computational cost.

The algorithm demonstrates a thorough understanding of Gaussian blur and implements it
efficiently using a two-pass approach.
*/
...
//very Important The (Blur) program takes about a minute to execute, because the blur radius = 30
// to shows blurred more clear to you, so please wait a minute and Thank you very much.
const int blurRadius = 30; // Constant blur radius
            ...
            for (int y = 0; y < newheight; ++y) {
                for (int x = 0; x < newwidth; ++x) {
                    for (int k = 0; k<3; k++){
                        ...
                        unsigned char& pixelColor = image.getPixel(originalX, originalY, k);
